...
impl Drain {
    ...
    pub fn add_log_message(&mut self, content: &str) -> (LogCluster, String) {
        ...
        match match_result {
            Some(cluster_id) => {
                 ...
                 let new_template_tokens = Self::create_template(&content_tokens, &cluster.log_template_tokens, &self.param_str);
    ...
    fn tree_search(
        ...
        Self::fast_match(id_to_cluster, &cur_node.cluster_ids, tokens, sim_th, include_params, param_str)
    }
    ...
    fn fast_match(
        ...
        for &cluster_id in cluster_ids {
            if let Some(cluster) = id_to_cluster.get(&cluster_id) {
                let (cur_sim, param_count) = Self::get_seq_distance(&cluster.log_template_tokens, tokens, include_params, param_str);
    ...
    fn get_seq_distance(seq1: &[String], seq2: &[String], include_params: bool, param_str: &str) -> (f64, i32) {
    ...
    fn create_template(seq1: &[String], seq2: &[String], param_str: &str) -> Vec<String> {
         seq1.iter().zip(seq2.iter()).map(|(t1, t2)| {
             if t1 == t2 {
                 t2.clone()
             } else {
                 param_str.to_string()
    ...
    fn add_seq_to_prefix_tree(
        ...
        let first_layer_node = root_node.key_to_child_node
             ...
             if cur_node.key_to_child_node.contains_key(token) {
                 ...
                 if has_numbers {
                     if !cur_node.key_to_child_node.contains_key(param_str) {
                         cur_node.key_to_child_node.insert(param_str.to_string(), Node::new());
                     }
                     cur_node = cur_node.key_to_child_node.get_mut(param_str).unwrap();
                 } else {
                     if cur_node.key_to_child_node.contains_key(param_str) {
                          if cur_node.key_to_child_node.len() < max_children {
                               cur_node.key_to_child_node.insert(token.clone(), Node::new());
                               cur_node = cur_node.key_to_child_node.get_mut(token).unwrap();
                          } else {
                               cur_node = cur_node.key_to_child_node.get_mut(param_str).unwrap();
                          }
